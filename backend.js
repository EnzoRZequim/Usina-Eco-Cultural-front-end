...
// Rota para acessar dados diários (adicionada)
app.get('/acessos/diarios', async (req, res) => {
    try {
        const dados = await ContadorAcessos.aggregate([
            {
                $project: {
        ...
        console.log("Erro ao obter dados diários:", erro);
        res.status(500).json({ error: "Erro ao obter dados diários" });
    }
});

// Rota para acessar dados semanais
app.get('/acessos/semanais', async (req, res) => {
    try {
        const dados = await ContadorAcessos.aggregate([
            {
                $group: {
            ...
            { $sort: { _id: 1 } } // Ordena por semana
        ]);
        res.status(200).json(dados);
    } catch (erro) {
        console.log("Erro ao obter dados semanais:", erro);
        res.status(500).json({ error: "Erro ao obter dados semanais" });
    }
});

// Rota para acessar dados mensais
app.get('/acessos/mensais', async (req, res) => {
    try {
        const dados = await ContadorAcessos.aggregate([
            {
                $group: {
        ...
        console.log("Erro ao obter dados mensais:", erro);
        res.status(500).json({ error: "Erro ao obter dados mensais" });
    }
});
// Rota para acessar dados mensais (com detalhes dos dias)
app.get('/acessos/mensais/dia', async (req, res) => {
    try {
        const dados = await ContadorAcessos.aggregate([
            {
                // Formata a data para ano e mês (exemplo: 2024-11)
                $project: {
                    anoMes: { $dateToString: { format: "%Y-%m", date: "$data" } },
                    dia: { $dayOfMonth: "$data" },
                    count: 1
                }
            },
            {
                ...
                $group: {
                    _id: { anoMes: "$anoMes", dia: "$dia" },
                    total: { $sum: "$count" }
                }
            },
            {
                // Ordena os dados por ano, mês e dia
                $sort: { "_id.anoMes": 1, "_id.dia": 1 }
            }
        ]);
        res.status(200).json(dados);
    } catch (erro) {
        console.log("Erro ao obter dados mensais diários:", erro);
        res.status(500).json({ error: "Erro ao obter dados mensais diários" });
    }
});
